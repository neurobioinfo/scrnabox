...
if [[ $QUEUE =~ bash ]] && [[  ${MODE0[@]}  =~  1  ]]; then
  ...
  bash ${PIPELINE_HOME}/scrna/scripts/step1/create_cellranger_scrna_local.sh $OUTPUT_DIR /job_info/.tmp/scrna_cellranger_local.sh $OUTPUT_DIR/job_info/.tmp/step1_par.txt

  while read item
  do
    # cp ${PIPELINE_HOME}/scrna/scripts/step1/slurm.template $item
    # sed -i $item/slurm.template -e 's/account0/'$ACCOUNT'/'
    cp $OUTPUT_DIR/job_info/.tmp/scrna_cellranger_local.sh $item
  done < ${OUTPUT_DIR}/job_info/.tmp/sample_dir.list

  export OUTPUT_DIR=$OUTPUT_DIR
  while read item
  do
      cd ${item}; bash  ${item}/scrna_cellranger_local.sh -r ouput_folder &
  done < ${OUTPUT_DIR}/job_info/.tmp/sample_dir.list
...
# if [[ $QUEUE =~ bash ]] && [[  ${MODE0[@]}  =~  2  ]]; then
  ...
  $QUEUE $PIPELINE_HOME/scrna/scripts/step2/pipeline_step2.sh --export=OUTPUT_DIR=${OUTPUT_DIR},PIPELINE_HOME=${PIPELINE_HOME},R_LIB_PATH=${R_LIB_PATH},R_VERSION=${R_VERSION},SCRNA_METHOD=${SCRNA_METHOD}  &> $JOB_OUTPUT_DIR/logs/${STEP}.$(date +%FT%H.%M.%S) 
  echo "-------------------------------------------" >> $EXPECTED_DONE_FILES
...
if [[ $QUEUE =~ bash ]] && [[  ${MODE0[@]}  =~  3 ]]  ; then
  ...
  $QUEUE $PIPELINE_HOME/scrna/scripts/step3/pipeline_step3.sh --export=OUTPUT_DIR=${OUTPUT_DIR},PIPELINE_HOME=${PIPELINE_HOME},R_LIB_PATH=${R_LIB_PATH},R_VERSION=${R_VERSION} &> $JOB_OUTPUT_DIR/logs/${STEP}.$(date +%FT%H.%M.%S) 
...
if [[ $QUEUE =~ bash ]] &&  [[  ${MODE0[@]}  =~  4 ]]  ; then
  ...
  $QUEUE $PIPELINE_HOME/scrna/scripts/step4/pipeline_step4.sh --export=OUTPUT_DIR=${OUTPUT_DIR},PIPELINE_HOME=${PIPELINE_HOME},R_LIB_PATH=${R_LIB_PATH},R_VERSION=${R_VERSION},SCRNA_METHOD=${SCRNA_METHOD} &> $JOB_OUTPUT_DIR/logs/${STEP}.$(date +%FT%H.%M.%S) 
  echo "-------------------------------------------" >> $EXPECTED_DONE_FILES
...
if [[ $QUEUE =~ bash ]] && [[  ${MODE0[@]}  =~  5 ]]  ; then
  ...
  echo "STEP 4 submitted "  >> $EXPECTED_DONE_FILES
  $QUEUE $PIPELINE_HOME/scrna/scripts/step5/pipeline_step5.sh  --export=OUTPUT_DIR=${OUTPUT_DIR},PIPELINE_HOME=${PIPELINE_HOME},R_LIB_PATH=${R_LIB_PATH},R_VERSION=${R_VERSION},SCRNA_METHOD=${SCRNA_METHOD} &> $JOB_OUTPUT_DIR/logs/${STEP}.$(date +%FT%H.%M.%S) 
  echo "-------------------------------------------" >> $EXPECTED_DONE_FILES  
...
if [[ $QUEUE =~ bash ]] && [[  ${MODE0[@]}  =~  6 ]]  ; then
  ...
  $QUEUE $PIPELINE_HOME/scrna/scripts/step6/pipeline_step6.sh --export=OUTPUT_DIR=${OUTPUT_DIR},PIPELINE_HOME=${PIPELINE_HOME},R_LIB_PATH=${R_LIB_PATH},R_VERSION=${R_VERSION},SCRNA_METHOD=${SCRNA_METHOD} &> $JOB_OUTPUT_DIR/logs/${STEP}.$(date +%FT%H.%M.%S) 
  echo "-------------------------------------------" >> $EXPECTED_DONE_FILES    
...
if [[ $QUEUE =~ bash ]] && [[  ${MODE0[@]}  =~  7  ]] && [[   ${STEP7markergsea}  =~  T ]]; then
  ...
  $QUEUE $PIPELINE_HOME/scrna/scripts/step7/pipeline_step7_markergsea.sh --export=OUTPUT_DIR=${OUTPUT_DIR},PIPELINE_HOME=${PIPELINE_HOME},R_LIB_PATH=${R_LIB_PATH},R_VERSION=${R_VERSION},SCRNA_METHOD=${SCRNA_METHOD} &> $JOB_OUTPUT_DIR/logs/${STEP}.$(date +%FT%H.%M.%S) 
  echo "-------------------------------------------" >> $EXPECTED_DONE_FILES  
...
if [[ $QUEUE =~ bash ]] && [[  ${MODE0[@]}  =~  7  ]] &&  [[   ${STEP7knownmarkers}  =~  T ]]; then
  echo "STEP 7 submitted "  >> $EXPECTED_DONE_FILES
  $QUEUE $PIPELINE_HOME/scrna/scripts/step7/pipeline_step7_knownmarkers.sh --export=OUTPUT_DIR=${OUTPUT_DIR},PIPELINE_HOME=${PIPELINE_HOME},R_LIB_PATH=${R_LIB_PATH},R_VERSION=${R_VERSION},SCRNA_METHOD=${SCRNA_METHOD}  &> $JOB_OUTPUT_DIR/logs/${STEP}.$(date +%FT%H.%M.%S) 
  echo "-------------------------------------------" >> $EXPECTED_DONE_FILES  
  echo "The Output is under ${OUTPUT_DIR}/step7" >> $EXPECTED_DONE_FILES
fi 
...
if [[ $QUEUE =~ bash ]] && [[  ${MODE0[@]}  =~  7  ]] &&  [[   ${STEP7referenceannotation}  =~  T ]]; then
  echo "STEP 7 submitted "  >> $EXPECTED_DONE_FILES
    $QUEUE $PIPELINE_HOME/scrna/scripts/step7/pipeline_step7_referenceannotation.sh --export=OUTPUT_DIR=${OUTPUT_DIR},PIPELINE_HOME=${PIPELINE_HOME},R_LIB_PATH=${R_LIB_PATH},R_VERSION=${R_VERSION},SCRNA_METHOD=${SCRNA_METHOD}  &> $JOB_OUTPUT_DIR/logs/${STEP}.$(date +%FT%H.%M.%S) 
  echo "-------------------------------------------" >> $EXPECTED_DONE_FILES
  echo "The Output is under ${OUTPUT_DIR}/step7" >> $EXPECTED_DONE_FILES
fi 
...
if [[ $QUEUE =~ bash ]] && [[  ${MODE0[@]}  =~  7  ]] &&  [[   ${STEP7annotate}  =~  T ]]; then
  echo "STEP 7 submitted "  >> $EXPECTED_DONE_FILES
  $QUEUE $PIPELINE_HOME/scrna/scripts/step7/pipeline_step7_annotate.sh --export=OUTPUT_DIR=${OUTPUT_DIR},PIPELINE_HOME=${PIPELINE_HOME},R_LIB_PATH=${R_LIB_PATH},R_VERSION=${R_VERSION},SCRNA_METHOD=${SCRNA_METHOD}  &> $JOB_OUTPUT_DIR/logs/${STEP}.$(date +%FT%H.%M.%S) 
  echo "-------------------------------------------" >> $EXPECTED_DONE_FILES
  echo "The Output is under ${OUTPUT_DIR}/step7" >> $EXPECTED_DONE_FILES
fi 
...
if [[ $QUEUE =~ bash ]] && [[  ${MODE0[@]}  =~  8  ]] &&  [[   ${STEP8addmeta}  =~  T ]]; then
  echo "STEP 8 submitted (not dependent on other job) "  >> $EXPECTED_DONE_FILES
    $QUEUE $PIPELINE_HOME/scrna/scripts/step8/pipeline_step8_addmeta.sh  --export=OUTPUT_DIR=${OUTPUT_DIR},PIPELINE_HOME=${PIPELINE_HOME},R_LIB_PATH=${R_LIB_PATH},R_VERSION=${R_VERSION},SCRNA_METHOD=${SCRNA_METHOD}  &> $JOB_OUTPUT_DIR/logs/${STEP}.$(date +%FT%H.%M.%S) 
    echo "-------------------------------------------" >> $EXPECTED_DONE_FILES    
...
if [[ $QUEUE =~ bash ]] && [[  ${MODE0[@]}  =~  8  ]] &&  [[  ${STEP8rundge}  =~  T ]]; then
    echo "STEP 8 submitted (not dependent on other job) "  >> $EXPECTED_DONE_FILES
    $QUEUE $PIPELINE_HOME/scrna/scripts/step8/pipeline_step8_rundge.sh --export=OUTPUT_DIR=${OUTPUT_DIR},PIPELINE_HOME=${PIPELINE_HOME},R_LIB_PATH=${R_LIB_PATH},R_VERSION=${R_VERSION},SCRNA_METHOD=${SCRNA_METHOD} &> $JOB_OUTPUT_DIR/logs/${STEP}.$(date +%FT%H.%M.%S) 
    echo "-------------------------------------------" >> $EXPECTED_DONE_FILES    
    echo "The Output is under ${OUTPUT_DIR}/step8" >> $EXPECTED_DONE_FILES
fi 
