...
if (contrast=="F") {
   dd<-read.csv(paste(output_dir,'/job_output/step8_contrast_main.txt',sep='/'), sep="")
} else {
...
run_func_main<- function(design,des,pheno,selvar,gp2){
   ...
   write.csv(aa,file=paste(output_dir,paste('/step8/cont_main',paste(des,'.csv',sep=''),sep='/'),sep=''))
}
