...
if (contrast=="F") {
   dd<-read.csv(paste(output_dir,'/job_output/step8_contrast_inte.txt',sep='/'), sep="")
} else {
...
#######
seu.int.c<-readRDS(paste(output_dir,'/step8/objs',"/seu.int.c.rds", sep=""))
dge<-readRDS(paste(output_dir,'/step8/objs',"/dge.rds", sep=""))
...
run_func_inte<- function(design,des,maincell,pheno,selvar,gp2){
   ...
   write.csv(aa,file=paste(output_dir,paste('/step8/cont_inte',paste(des,'.csv',sep=''),sep='/'),sep=''))
}
