...
### create directories
## base directory
OUT_dir_sample <- paste(output_dir,"/step8/sample_sample_wilcoxon",sep='') 
...
### create directories
## base directory
OUT_dir_sample <- paste(output_dir,"/step8/sample_cell_wilcoxon",sep='') 
dir.create(OUT_dir_sample)
...
if (tolower(par_run_pseudo_bulk)=='yes') {
...
dd<-read.csv(paste(output_dir,'/job_info/parameters/step8_contrast_pseudo_bulk.txt',sep='/'), sep="")
## store the contrast name
cont_name <- dd[1,1]
## create a psudobulk folder
sudo_dir <- paste(output_dir,'/step8/pseudo_bulk', sep = "")
dir.create(sudo_dir)
outputpath <- sudo_dir
...
## write session info
writeLines(capture.output(sessionInfo()), paste(output_dir,'/step8/info8/sessionInfo_pseudo_bulk.txt', sep=""))
if(file.exists("Rplots.pdf")){
    file.remove("Rplots.pdf")
}
