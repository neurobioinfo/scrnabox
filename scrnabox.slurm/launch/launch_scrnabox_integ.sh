...
if [[ $QUEUE =~ sbatch ]] && [[  ${MODE0[@]}  =~  integrate  ]] ; then
  ...
  step_integrate="$QUEUE -A $ACCOUNT  \
    ...
    --time=${WALLTIME} \
    --job-name $STEP \
    --export OUTPUT_DIR=${OUTPUT_DIR},PIPELINE_HOME=${PIPELINE_HOME},R_LIB_PATH=${R_LIB_PATH},R_VERSION=${R_VERSION},SEULIST=${SEULIST} \
    --output $JOB_OUTPUT_DIR/logs/%x.o%j \
    $PIPELINE_HOME/integrate/pipeline_integrate.qsub"
